	...
	*/
	...
	function ScoreBox(props){
		return(
			<div className={props.className}>
				<div className="scoreBoxLabel"> Score  <span> {props.score}  </span> </div>
	...
	class Timer extends React.Component{
		...
		render(){
			return(
			<div className="timer"> 
				<div className="timerLabel"> Time <span>{this.state.curTime} </span></div>
			</div>
			);
		}
	...
	class Game extends React.Component{
		...
		handleClick(i) {
			...
			if(arr.indexOf(i) > -1){
				...
				if(this.state.lastClicked == -1){ //If this is first click no need to check neighbor
						isNeighbor = true;					
				}else{
		...
		checkClick(){
			...
			if(this.state.curWord.length > 2 ){
				if(isCorrect){ //Check if spelling is corret
		...
		render() {
			...
			<div className="gameContainer">
				...
				<Board
					charlist={this.state.charList} clickArr={this.state.clickStateArr}
...
const arr = this.props.clickArr;
		  //	arr = this.props.clickArr;
			if(arr[i]==0){
					return(
					<Square
				curVal={this.props.charlist[i]} className="square"
